...
export function CustomEdge({
  ...
  const sourceVertex = map.get(source)!
  const targetVertex = map.get(target)!
  let path = customEdgePaths.get(key) ?? ""

  if (!path) {
    const yDirection = sourceY - targetY > 0 ? -1 : 1

    if (sourceVertex.coReqs.includes(targetVertex.courseCode)) {
      const xDirection = targetY - sourceY > 0 ? -1 : 1
      path += `
        M${sourceX - (xDirection === 1 ? 0 : NODE_WIDTH)},${sourceY}
        l${xDirection * 0.25 * NODE_WIDTH},0
        l0,${targetY - sourceY}
        l${xDirection * -0.25 * NODE_WIDTH},0`
    } else {
