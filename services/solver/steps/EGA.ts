...
async function Mutate(curriculum: Curriculum, alreadyAttemptedMoves: string[]): Promise<Curriculum> {
	...
	while (alreadyAttemptedMoves.length < courses.length) {
		...
		const minSemesterIndex = tempCurriculum.semesters.flat().filter(v => courseToMove.preReqs.includes(v.courseCode)).filter((v, i, a) => a.findIndex(v2 => v2.semester === v.semester) === i).length
