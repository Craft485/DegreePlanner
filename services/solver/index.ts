import { GenerateHexCodeFromCourseCode } from './utils/color.js'
import * as Analytics from 'types/solver.js'
import { OptimizeCurriculum } from './steps/optimize.js'
import { MergeCurricula } from './steps/merge.js'
import { readFile, writeFile } from 'node:fs/promises'
import { CalculateMetrics } from './steps/metrics.js'
import { Optimize as EGA } from './steps/EGA.js'
import { Builder } from './steps/Builder.js'
...
export async function ParseAnalytics(programStacks: string[]): Promise<Analytics.Curriculum> {
    ...
    for await (const programStack of programStacks) {
        const fileContents = await readFile(`../solver/data/${programStack}.json`, 'utf-8').catch(err => { if (err) throw err })
    ...
    if (curricula.length === 1) {
        ...
        await writeFile('./buffer.ignore.json', JSON.stringify(curricula[0]), { encoding: 'utf-8' })    
        return curricula[0]
    }
    ...
    await writeFile('./merged-buffer.ignore.json', JSON.stringify(mergedCurricula), { encoding: 'utf-8' })
    ...
    await writeFile('./buffer.ignore.json', JSON.stringify(optimizedDegreePlan), { encoding: 'utf-8' })
    const finalDegreePlan: Analytics.Curriculum = await EGA([optimizedDegreePlan])
    await writeFile('./EGA-buffer.ignore.json', JSON.stringify(finalDegreePlan), { encoding: 'utf-8' })
